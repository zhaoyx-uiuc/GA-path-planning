...
from utils import  cal_idx_from_coord, greedy_minimize_rows, calculate_coverage,Write3dcoord_toply, Write3dcoord_topath, output_visualizable_faces, waypoint_to_center, random_rotate, find_pairs
import os
import torch
import time
...
data_path='safedist=0.5_visdist='+str(vis_dist)+'_step='+str(step)+'/'
...
def ga_opt(rule_proportion, population_size, exe_rate, new_proportion, happen_rate, tournament_rate, coverage_thres,save_path):
    ...
    TOURNAMENT_SIZE = 25#int(0.1*POPULATION_SIZE)
    NUM_GENERATIONS=500
    ...
    if ifcontinue==False:
        ...
        remaining_generation=NUM_GENERATIONS
        fitness_record=[]
    else:
        population=np.load(save_path+'popolation.npy',allow_pickle=True)
        fitness_record=np.loadtxt(save_path+'fitness.txt').tolist()
        remaining_generation=NUM_GENERATIONS-len(fitness_record)
        
    for generation in range(remaining_generation):
        ...
        np.save(save_path+'popolation',np.asarray(population))
...
exe_rate_list=[0.1] #default:0.1
new_proportion_list=[0] #no change
happen_rate_list=[0.75] #default:0.75
tournament_rate_list=[0.1] #default:0.1
coverage_list=[0.8] #default:0.95
population_list=[250] #default:250
rule_list=[0.5] #default:0.5
                    ...
                    for exe_rate in exe_rate_list:
                        for population in population_list:
                            for rule_proportion in rule_list:
                                ga_opt(rule_proportion, population, exe_rate, new_proportion, happen_rate, tournament_rate, coverage,'test_samets/ruleinit='+str(rule_proportion)+'_KAPPA=0.1_FOV=120_dist=10_ind='+str(population)+'_gen=1000/'+str(exe_rate)+'_'+str(new_proportion)+'_'+str(happen_rate)+'_'+str(tournament_rate)+'_'+str(coverage)+'/'+str(idx+1)+'/')
